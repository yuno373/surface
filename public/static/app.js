...
async function loadSettings(){const c=document.getElementById('settings-list');if(!c)return;try{const r=await api('/api/auth/profile');let inst='';if(deferredPrompt)inst='<button onclick="installPWA()" class="w-full text-center text-green-600 py-3 font-semibold"><i class="fas fa-download mr-2"></i>アプリをインストール</button>';const roles=currentUser.roles||[currentUser.role];const isStaff=roles.some(rr=>['admin','teacher'].includes(rr));const isAdmin=roles.includes('admin');c.innerHTML='<div class="card p-4"><h3 class="font-bold mb-3">アカウント情報</h3><div class="space-y-3"><div class="flex justify-between items-center"><span class="text-sm text-gray-600">名前</span><span class="text-sm font-semibold">'+esc(r.user.name)+'</span></div><div class="flex justify-between items-center"><span class="text-sm text-gray-600">ログインID</span><span class="text-sm">'+esc(r.user.login_id||'-')+'</span></div><div class="flex justify-between items-center"><span class="text-sm text-gray-600">学年</span><span class="text-sm">'+esc(r.user.grade||'-')+'</span></div><div class="flex justify-between items-center"><span class="text-sm text-gray-600">クラス</span><span class="text-sm">'+esc(r.user.class_num||'-')+'</span></div></div></div><div class="card p-4"><h3 class="font-bold mb-3">通知設定</h3><div id="notif-settings-content"><div class="skeleton h-12"></div></div></div>'+(isAdmin?'<div class="card p-4"><h3 class="font-bold mb-3">管理設定</h3><div id="admin-settings-content"><div class="skeleton h-12"></div></div></div>':'')+inst+'<button onclick="logout()" class="w-full text-center text-red-500 py-3 mt-4 font-semibold"><i class="fas fa-sign-out-alt mr-2"></i>ログアウト</button>'+(isStaff?'<button onclick="renderAdmin(document.getElementById(\'tab-content\'))" class="w-full text-center text-blue-600 py-3 font-semibold"><i class="fas fa-shield-alt mr-2"></i>管理パネル</button>':'')+'<div class="text-center text-xs text-gray-400 py-4">上中黒板 v2.0</div>';loadNotifSettings();if(isAdmin)loadAdminSettings();}catch{c.innerHTML='<div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>読込失敗</p></div>';}}

async function loadAdminSettings(){const c=document.getElementById('admin-settings-content');if(!c)return;try{const r=await api('/api/admin/settings');const s=r.settings;c.innerHTML='<div class="space-y-2">'+[['teacher_can_users','先生がユーザー管理を表示'],['teacher_can_posts','先生が投稿管理を表示'],['teacher_can_bulk','先生が一括生成を表示'],['notif_self_default','自分通知を標準で有効']].map(([k,lb])=>{const on=s[k]==='true';return'<div class="flex items-center justify-between"><span class="text-sm text-gray-700">'+lb+'</span><div class="toggle'+(on?' on':'')+'" onclick="toggleAdminSetting(\''+k+'\',this)"></div></div>';}).join('')+'</div>';}catch{c.innerHTML='<p class="text-sm text-gray-400">読込失敗</p>';}}
...
function renderAdminFull(container){
  ...
  const isStaff=roles.some(r=>['admin','teacher'].includes(r));
  if(!isStaff){container.innerHTML='<div class="empty-state"><i class="fas fa-lock"></i><p>管理者のみアクセス可能</p></div>';return;}
  const tabs=isAdmin?'<button onclick="switchAdminTab(\'users\',this)" class="sub-nav-btn active">ユーザー管理</button><button onclick="switchAdminTab(\'roles\',this)" class="sub-nav-btn">ロール管理</button><button onclick="switchAdminTab(\'tokens\',this)" class="sub-nav-btn">招待コード</button><button onclick="switchAdminTab(\'bulk\',this)" class="sub-nav-btn">一括生成</button><button onclick="switchAdminTab(\'posts\',this)" class="sub-nav-btn">投稿管理</button><button onclick="switchAdminTab(\'stats\',this)" class="sub-nav-btn">統計</button><button onclick="switchAdminTab(\'diag\',this)" class="sub-nav-btn">診断</button>':'<button onclick="switchAdminTab(\'tokens\',this)" class="sub-nav-btn active">招待コード</button><button onclick="switchAdminTab(\'stats\',this)" class="sub-nav-btn">統計</button>';
...
function switchAdminTab(tab,btn){document.querySelectorAll('.sub-nav-btn').forEach(b=>b.classList.remove('active'));btn.classList.add('active');if(tab==='users')loadAdminUsers();else if(tab==='roles')loadAdminRoles();else if(tab==='tokens')loadAdminTokens();else if(tab==='bulk')loadAdminBulkCreate();else if(tab==='posts')loadAdminPosts();else if(tab==='stats')loadAdminStats();else if(tab==='diag')loadAdminDiagnostics();}
