...
async function fetchDisasterInfo(){try{const r=await api('/api/disaster/current');const el=document.getElementById('disaster-text');const bar=document.getElementById('disaster-bar');if(el&&r.title){el.innerHTML='<i class=\"fas fa-shield-alt mr-1\"></i>'+esc(r.title);if(bar){bar.classList.remove('hidden','level2','level3','level4','level5');if(r.level>=5)bar.classList.add('level5');else if(r.level>=4)bar.classList.add('level4');else if(r.level>=3)bar.classList.add('level3');else if(r.level>=2)bar.classList.add('level2');}}else if(el){el.innerHTML='<i class=\"fas fa-shield-alt mr-1\"></i>防災情報: 現在警報はありません';if(bar)bar.classList.remove('level2','level3','level4','level5');}try{if(el&&r.title&&r.level>=3&&Notification.permission==='granted'&&!sessionStorage.getItem('disaster-notified')){new Notification('上中黒板 - 気象警報',{body:r.title,icon:'/icons/icon-192.png'});sessionStorage.setItem('disaster-notified','1');}}catch{}}catch{const el=document.getElementById('disaster-text');if(el)el.innerHTML='<i class=\"fas fa-shield-alt mr-1\"></i>防災情報: 現在警報はありません';const bar=document.getElementById('disaster-bar');if(bar)bar.classList.remove('level2','level3','level4','level5');}}
