...
async function showWeatherModal(){showModal('天気予報','<div class="skeleton h-48"></div>',[]);try{const r=await api('/api/weather/forecast');const f=r.forecasts?.[0];if(!f){document.getElementById('modal-body').innerHTML='<p class="text-sm text-gray-400 p-4">読込失敗</p>';return;}const pops=f.pops||[];const wd=f.weather||'';const wd2=f.weatherTomorrow||'';const h='<div class="space-y-4 p-2">'+(wd?'<div class="card p-4"><div class="text-lg font-bold mb-1">今日の天気</div><p class="text-sm leading-relaxed">'+wd.split(/　/).filter(Boolean).map(function(s){return'<span class="inline-block bg-gray-100 rounded px-2 py-0.5 text-xs mr-1 mb-1">'+esc(s)+'</span>'}).join('').replace(/\u3000/g,'<br>')+'</p>'+(f.wind?'<p class="text-xs text-gray-500 mt-1">風: '+esc(f.wind)+'</p>':'')+'</div>':'')+(pops.length?'<div class="card p-4"><div class="text-sm font-bold mb-2">降水確率</div><div class="grid grid-cols-4 gap-1 text-center">'+['6-12時','12-18時','18-24時','24-6時'].map((t,i)=>'<div><div class="text-xs text-gray-500">'+t+'</div><div class="text-xl font-bold'+(parseInt(pops[i])>=50?' text-blue-600':'')+'">'+(pops[i]||'-')+'%</div></div>').join('')+'</div></div>':'')+(f.temps?'<div class="card p-4"><div class="text-sm font-bold mb-2">気温</div><div class="grid grid-cols-2 gap-2 text-center">'+(f.temps.today?'<div><div class="text-xs text-gray-500">日中</div><div class="text-xl font-bold">'+f.temps.today+'°C</div></div>':'')+(f.temps.tonight?'<div><div class="text-xs text-gray-500">夜</div><div class="text-xl font-bold">'+f.temps.tonight+'°C</div></div>':'')+'</div></div>':'')+(wd2?'<div class="card p-4"><div class="text-sm font-bold mb-1">明日の天気</div><p class="text-sm leading-relaxed">'+wd2.split(/　/).filter(Boolean).map(function(s){return'<span class="inline-block bg-gray-100 rounded px-2 py-0.5 text-xs mr-1 mb-1">'+esc(s)+'</span>'}).join('').replace(/\u3000/g,'<br>')+'</p>'+(f.windTomorrow?'<p class="text-xs text-gray-500 mt-1">風: '+esc(f.windTomorrow)+'</p>':'')+'</div>':'')+(f.publishingOffice?'<p class="text-xs text-gray-400 text-center">'+esc(f.publishingOffice)+' | '+(f.reportDatetime||'').slice(0,16).replace('T',' ')+'</p>':'')+'</div>';document.getElementById('modal-body').innerHTML=h;}catch{document.getElementById('modal-body').innerHTML='<p class="text-sm text-gray-400 p-4">読込失敗</p>';}}async function fetchUnreadCount(){try{const r=await api('/api/messages/unread-count');const badge=document.getElementById('msg-badge');if(badge){badge.textContent=r.count>0?(r.count>99?'99+':r.count):'';badge.classList.toggle('hidden',r.count===0);}}catch{}}
