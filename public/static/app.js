...
async function renderAdminTeacher(container){
  try{const sr=await api('/api/admin/settings');const s=sr.settings||{};let tabs='<button onclick="switchAdminTab(\'roles\',this)" class="sub-nav-btn active">権限管理</button><button onclick="switchAdminTab(\'tokens\',this)" class="sub-nav-btn">招待コード</button><button onclick="switchAdminTab(\'stats\',this)" class="sub-nav-btn">統計</button>';if(s.teacher_can_users==='true')tabs+='<button onclick="switchAdminTab(\'users\',this)" class="sub-nav-btn">ユーザー管理</button>';if(s.teacher_can_posts==='true')tabs+='<button onclick="switchAdminTab(\'posts\',this)" class="sub-nav-btn">投稿管理</button>';if(s.teacher_can_bulk==='true')tabs+='<button onclick="switchAdminTab(\'bulk\',this)" class="sub-nav-btn">一括生成</button>';container.innerHTML='<div class="bg-white border-b px-4 py-3"><h2 class="font-bold text-gray-800 flex items-center gap-2"><i class="fas fa-shield-alt text-red-600"></i>管理</h2></div><div class="p-4 space-y-4"><div class="flex flex-wrap gap-2">'+tabs+'</div><div id="admin-content"><div class="skeleton h-32"></div></div></div>';loadAdminRoles();}catch{renderAdminFull(container);}
}
