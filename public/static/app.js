...
async function loadPEChecklist(){var c=document.getElementById('pe-checklist-content');if(!c)return;try{var r=await api('/api/checklist/items');if(!r.items||!r.items.length){c.innerHTML='<div class="empty-state" style="text-align:center;padding:48px 24px;color:#95a5a6"><i class="fas fa-inbox" style="font-size:48px;margin-bottom:12px;display:block"></i><p>点検項目がありません</p></div>';return;}var total=r.items.length,checked=0;for(var i=0;i<total;i++){if(r.items[i].status)checked++;}var pct=total?Math.round(checked/total*100):0;var items=r.items.slice().sort(function(a,b){var oa=a.status?0:1,ob=b.status?0:1;if(oa!==ob)return ob-oa;return a.name.localeCompare(b.name);});var h='<div style="background:white;border-radius:14px;padding:16px;margin-bottom:16px;box-shadow:0 2px 8px rgba(0,0,0,0.06)"><div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:8px"><span style="font-weight:600">点検進捗</span><span>'+checked+'/'+total+' ('+pct+'%)</span></div><div style="background:#e5e7eb;border-radius:99px;height:8px;overflow:hidden"><div style="background:#22c55e;width:'+pct+'%;height:100%;border-radius:99px;transition:width 0.3s"></div></div></div>';h+='<div style="display:flex;flex-direction:column;gap:10px">';for(var i=0;i<items.length;i++){var it=items[i];var s=it.status==='ok'?'✓ OK':it.status==='ng'?'✗ NG':'未チェック';var sc=it.status==='ok'?'color:#16a34a;background:#dcfce7':it.status==='ng'?'color:#dc2626;background:#fef2f2':'color:#6b7280;background:#f3f4f6';var avail=it.total_count-(it.active_rentals||0);h+='<div style="background:white;border-radius:14px;padding:16px;box-shadow:0 2px 8px rgba(0,0,0,0.06)"><div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:8px"><span style="font-weight:bold;font-size:14px">'+esc(it.name)+'</span><span style="font-size:11px;padding:2px 10px;border-radius:99px;'+sc+'">'+s+'</span></div>'+(it.location?'<p style="font-size:12px;color:#95a5a6;margin-bottom:6px">'+esc(it.location)+'</p>':'')+'<div style="display:flex;gap:12px;font-size:12px;color:#6b7280;margin-bottom:10px"><span><i class="fas fa-cubes" style="margin-right:2px"></i>総数 '+it.total_count+'</span>'+(it.active_rentals?'<span><i class="fas fa-hand-holding" style="margin-right:2px"></i>貸出中 '+it.active_rentals+'</span>':'')+'<span><i class="fas fa-check-circle" style="margin-right:2px"></i>残り '+avail+'</span></div>'+(it.can_check?'<div style="display:flex;gap:8px"><button class="pecheck-ok" data-id="'+it.id+'" style="background:#22c55e;color:white;padding:6px 20px;border:none;border-radius:99px;font-size:13px;font-weight:600;cursor:pointer">OK</button><button class="pecheck-ng" data-id="'+it.id+'" style="background:#ef4444;color:white;padding:6px 20px;border:none;border-radius:99px;font-size:13px;font-weight:600;cursor:pointer">NG</button></div>':'<p style="font-size:12px;color:#95a5a6">最終確認: '+(it.last_checker?esc(it.last_checker):'')+(it.last_checked?' / '+formatRelative(it.last_checked):'')+'</p>')+'</div>';}h+='</div>';c.innerHTML=h;c.onclick=function(e){var ok=e.target.closest('.pecheck-ok');if(ok){checkPEItem(parseInt(ok.getAttribute('data-id')),'ok');return;}var ng=e.target.closest('.pecheck-ng');if(ng){checkPEItem(parseInt(ng.getAttribute('data-id')),'ng');return;}};}catch{c.innerHTML='<div class="empty-state" style="text-align:center;padding:48px 24px;color:#95a5a6"><i class="fas fa-exclamation-circle" style="font-size:48px;margin-bottom:12px;display:block"></i><p>読込失敗</p></div>';}}
...
function renderAdminFull(container){
  ...
  const isStaff=roles.some(r=>['admin','teacher'].includes(r));
  if(!isStaff){container.innerHTML='<div class="empty-state"><i class="fas fa-lock"></i><p>管理者のみアクセス可能</p></div>';return;}
  const tabs=isAdmin?'<button onclick="switchAdminTab(\'users\',this)" class="sub-nav-btn active">ユーザー管理</button><button onclick="switchAdminTab(\'roles\',this)" class="sub-nav-btn">権限管理</button><button onclick="switchAdminTab(\'profile\',this)" class="sub-nav-btn">承認待ち</button><button onclick="switchAdminTab(\'tokens\',this)" class="sub-nav-btn">招待コード</button><button onclick="switchAdminTab(\'bulk\',this)" class="sub-nav-btn">一括生成</button><button onclick="switchAdminTab(\'posts\',this)" class="sub-nav-btn">投稿管理</button><button onclick="switchAdminTab(\'broadcast\',this)" class="sub-nav-btn">通知配信</button><button onclick="switchAdminTab(\'stats\',this)" class="sub-nav-btn">統計</button><button onclick="switchAdminTab(\'diag\',this)" class="sub-nav-btn">診断</button>':'<button onclick="switchAdminTab(\'roles\',this)" class="sub-nav-btn active">権限管理</button><button onclick="switchAdminTab(\'tokens\',this)" class="sub-nav-btn">招待コード</button><button onclick="switchAdminTab(\'stats\',this)" class="sub-nav-btn">統計</button>';
...
async function loadAdminBroadcast(){const c=document.getElementById('admin-content');if(!c)return;c.innerHTML='<div class="card p-4"><h3 class="font-bold mb-3">全体通知配信</h3><div class="space-y-3"><div><label class="form-label">タイトル</label><input id="bc-title" type="text" class="form-input" placeholder="例: お知らせ"></div><div><label class="form-label">内容</label><textarea id="bc-body" class="form-input" rows="3" placeholder="通知内容"></textarea></div><button onclick="submitBroadcast()" class="bg-blue-600 text-white px-6 py-2 rounded-full text-sm font-semibold"><i class="fas fa-bullhorn mr-1"></i>全員に配信</button><p class="text-xs text-gray-400 mt-2">プッシュ通知が有効なユーザーにはブラウザ通知も送信されます</p></div></div>';}
async function submitBroadcast(){const title=document.getElementById('bc-title').value.trim();const body=document.getElementById('bc-body').value.trim();if(!title||!body){toast('タイトルと内容を入力','error');return;}try{const r=await api('/api/admin/notifications/broadcast',{method:'POST',body:{title,body,type:'normal'}});if(r.error){toast(r.error,'error');return;}toast('通知を配信しました（'+r.sent+'件）','success');document.getElementById('bc-title').value='';document.getElementById('bc-body').value='';}catch(e){toast('失敗: '+(e.message||'エラー'),'error');}}
