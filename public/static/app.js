...
function renderPEChecklist(container) {
  ...
  navDiv.style.cssText='display:flex;gap:8px;margin-bottom:16px';
  navDiv.innerHTML='<button class="sub-nav-btn active" data-pechecktab="check">点検</button><button class="sub-nav-btn" data-pechecktab="history">履歴</button><button class="sub-nav-btn" data-pechecktab="rentals">貸出</button>';
...
async function loadPEChecklist(){var c=document.getElementById('pe-checklist-content');if(!c)return;try{var r=await api('/api/checklist/items');if(!r.items||!r.items.length){c.innerHTML='<div class="empty-state" style="text-align:center;padding:48px 24px;color:#95a5a6"><i class="fas fa-inbox" style="font-size:48px;margin-bottom:12px;display:block"></i><p>点検項目がありません</p></div>';return;}c.innerHTML='<div style="display:flex;flex-direction:column;gap:12px">'+r.items.map(function(i){var s=i.status==='ok'?'✓ OK':i.status==='ng'?'✗ NG':'未チェック';var sc=i.status==='ok'?'color:green':i.status==='ng'?'color:red':'color:#95a5a6';var h='<div style="background:white;border-radius:14px;padding:16px;box-shadow:0 2px 8px rgba(0,0,0,0.06)"><div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:8px"><span style="font-weight:bold;font-size:14px">'+esc(i.name)+'</span><span style="font-size:12px;'+sc+'">'+s+'</span></div><p style="font-size:12px;color:#95a5a6;margin-bottom:12px">'+esc(i.location||'')+'</p>';if(i.can_check){h+='<div style="display:flex;gap:8px"><button class="pecheck-ok" data-id="'+i.id+'" style="background:#22c55e;color:white;padding:4px 16px;border:none;border-radius:99px;font-size:12px;cursor:pointer">OK</button><button class="pecheck-ng" data-id="'+i.id+'" style="background:#ef4444;color:white;padding:4px 16px;border:none;border-radius:99px;font-size:12px;cursor:pointer">NG</button></div>';}else{h+='<p style="font-size:12px;color:#95a5a6">最終確認: '+(i.last_checker?esc(i.last_checker):'')+(i.last_checked?' / '+formatRelative(i.last_checked):'')+'</p>';}h+='</div>';return h;}).join('')+'</div>';c.onclick=function(e){var ok=e.target.closest('.pecheck-ok');if(ok){checkPEItem(parseInt(ok.getAttribute('data-id')),'ok');return;}var ng=e.target.closest('.pecheck-ng');if(ng){checkPEItem(parseInt(ng.getAttribute('data-id')),'ng');return;}};}catch{c.innerHTML='<div class="empty-state" style="text-align:center;padding:48px 24px;color:#95a5a6"><i class="fas fa-exclamation-circle" style="font-size:48px;margin-bottom:12px;display:block"></i><p>読込失敗</p></div>';}}
