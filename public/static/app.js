...
function openProfileEdit(){const u=currentUser;showModal('プロフィール編集','<div class="space-y-3"><div><label class="form-label">名前</label><input id="pe-name" type="text" class="form-input" value="'+esc(u.name)+'"></div><div><label class="form-label">部活動</label><input id="pe-club" type="text" class="form-input" value="'+esc(u.club||'')+'" placeholder="例: サッカー部"></div><div><label class="form-label">委員会</label><input id="pe-committee" type="text" class="form-input" value="'+esc(u.committee||'')+'" placeholder="例: 生徒会"></div><div><label class="form-label">自己紹介</label><textarea id="pe-bio" class="form-input" rows="3" placeholder="自己紹介を入力">'+esc(u.bio||'')+'</textarea></div></div>',[{label:'キャンセル',className:'border border-gray-300 text-gray-600 px-4 py-2 rounded-xl',action:closeModal},{label:'保存',className:'bg-green-600 text-white px-6 py-2 rounded-xl font-semibold',action:submitProfileEdit}]);}
async function submitProfileEdit(){try{const name=document.getElementById('pe-name').value.trim();if(!name){toast('名前は必須','error');return;}await api('/api/auth/profile',{method:'PUT',body:{name,club:document.getElementById('pe-club').value.trim()||null,committee:document.getElementById('pe-committee').value.trim()||null,bio:document.getElementById('pe-bio').value.trim()||null}});closeModal();toast('保存しました','success');loadSettings();}catch(e){toast(e.message||'失敗','error');}}
