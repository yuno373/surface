...
async function loadAdminStats(){const c=document.getElementById('admin-content');if(!c)return;try{const r=await api('/api/admin/stats');c.innerHTML='<div class="grid grid-cols-2 gap-3"><div class="card p-4 text-center"><p class="text-2xl font-bold text-green-600">'+r.total+'</p><p class="text-xs text-gray-500">総ユーザー数</p></div></div><div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4"><div class="card p-4"><h4 class="text-sm font-bold mb-2">学年別</h4><canvas id="stats-grade-chart" height="180" class="w-full"></canvas></div><div class="card p-4"><h4 class="text-sm font-bold mb-2">ロール別</h4><canvas id="stats-role-chart" height="180" class="w-full"></canvas></div><div class="card p-4"><h4 class="text-sm font-bold mb-2">部活動</h4><div id="stats-club-list" class="space-y-1"></div></div><div class="card p-4"><h4 class="text-sm font-bold mb-2">委員会</h4><div id="stats-committee-list" class="space-y-1"></div></div></div>';drawBarChart('stats-grade-chart',r.byGrade.map((x:any)=>({label:''+x.grade+'年',value:x.cnt})));drawBarChart('stats-role-chart',r.byRole.map((x:any)=>({label:{admin:'管理者',teacher:'先生',student:'生徒',captain:'部長',chairman:'委員長'}[x.role]||x.role,value:x.cnt})));document.getElementById('stats-club-list').innerHTML=r.byClub.map((x:any)=>'<div class="flex justify-between text-sm"><span>'+esc(x.club)+'</span><span class="font-bold">'+x.cnt+'</span></div>').join('');document.getElementById('stats-committee-list').innerHTML=r.byCommittee.map((x:any)=>'<div class="flex justify-between text-sm"><span>'+esc(x.committee)+'</span><span class="font-bold">'+x.cnt+'</span></div>').join('');}catch{c.innerHTML='<div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>読込失敗</p></div>';}}

function drawBarChart(canvasId,data){const cv=document.getElementById(canvasId);if(!cv)return;const ctx=cv.getContext('2d');const w=cv.parentElement.clientWidth||300;cv.width=w;cv.height=180;const barW=Math.max(6,w/data.length/2-2);const max=Math.max(...data.map((d:any)=>d.value),1);ctx.clearRect(0,0,w,180);data.forEach((d:any,i)=>{const x=i*(w/data.length)+(w/data.length-barW)/2;const h=(d.value/max)*140;ctx.fillStyle='#40916c';ctx.beginPath();ctx.roundRect(x,165-h,barW,h,3);ctx.fill();ctx.fillStyle='#666';ctx.font='9px sans-serif';ctx.textAlign='center';ctx.fillText(d.label,x+barW/2,178);ctx.fillStyle='#333';ctx.font='bold 10px sans-serif';ctx.fillText(d.value,x+barW/2,160-h);});}

async function loadAdminDiagnostics(){const c=document.getElementById('admin-content');if(!c)return;c.innerHTML='<div class="space-y-4"><div class="flex gap-2"><button onclick="loadAdminDiagnostics()" class="bg-blue-600 text-white px-4 py-1.5 rounded-full text-sm"><i class="fas fa-sync mr-1"></i>再診断</button></div><div id="diag-results"><div class="skeleton h-32"></div></div></div>';try{const r=await api('/api/admin/diagnostics');const l=document.getElementById('diag-results');l.innerHTML='<div class="space-y-2">'+r.checks.map((ch:any)=>{const icon={ok:'fa-check-circle text-green-500',error:'fa-exclamation-circle text-red-500',info:'fa-info-circle text-blue-500'}[ch.status]||'fa-circle text-gray-400';return'<div class="card p-3 flex items-center gap-3"><i class="fas '+icon+'"></i><div class="flex-1"><p class="text-sm font-semibold">'+esc(ch.name)+'</p><p class="text-xs text-gray-500">'+esc(ch.message)+'</p></div></div>';}).join('')+'</div>';}catch{const l=document.getElementById('diag-results');if(l)l.innerHTML='<div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>診断失敗</p></div>';}}
