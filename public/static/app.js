...
async function loadSettings(){const c=document.getElementById('settings-list');if(!c)return;try{const r=await api('/api/auth/profile');let inst='';if(deferredPrompt)inst='<button onclick="installPWA()" class="w-full text-center text-green-600 py-3 font-semibold"><i class="fas fa-download mr-2"></i>アプリをインストール</button>';const isStaff=(currentUser.roles||[currentUser.role]).some(rr=>['admin','teacher'].includes(rr));c.innerHTML='<div class="card p-4"><h3 class="font-bold mb-3">アカウント情報</h3><div class="space-y-3"><div class="flex justify-between items-center"><span class="text-sm text-gray-600">名前</span><span class="text-sm font-semibold">'+esc(r.user.name)+'</span></div><div class="flex justify-between items-center"><span class="text-sm text-gray-600">ログインID</span><span class="text-sm">'+esc(r.user.login_id||'-')+'</span></div><div class="flex justify-between items-center"><span class="text-sm text-gray-600">学年</span><span class="text-sm">'+esc(r.user.grade||'-')+'</span></div><div class="flex justify-between items-center"><span class="text-sm text-gray-600">クラス</span><span class="text-sm">'+esc(r.user.class_num||'-')+'</span></div></div></div><div class="card p-4"><h3 class="font-bold mb-3">通知設定</h3><div id="notif-settings-content"><div class="skeleton h-12"></div></div></div>'+inst+'<button onclick="logout()" class="w-full text-center text-red-500 py-3 mt-4 font-semibold"><i class="fas fa-sign-out-alt mr-2"></i>ログアウト</button>'+(isStaff?'<button onclick="renderAdmin(document.getElementById(\'tab-content\'))" class="w-full text-center text-blue-600 py-3 font-semibold"><i class="fas fa-shield-alt mr-2"></i>管理設定</button>':'')+'<div class="text-center text-xs text-gray-400 py-4">上中黒板 v2.0</div>';loadNotifSettings();}catch{c.innerHTML='<div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>読込失敗</p></div>';}}
...
function renderAdmin(container) {
  ...
  const tabs=isAdmin?'<button onclick="switchAdminTab(\'users\',this)" class="sub-nav-btn active">ユーザー管理</button><button onclick="switchAdminTab(\'roles\',this)" class="sub-nav-btn">ロール管理</button><button onclick="switchAdminTab(\'tokens\',this)" class="sub-nav-btn">招待コード</button><button onclick="switchAdminTab(\'stats\',this)" class="sub-nav-btn">統計</button><button onclick="switchAdminTab(\'diag\',this)" class="sub-nav-btn">診断</button>':'<button onclick="switchAdminTab(\'tokens\',this)" class="sub-nav-btn active">招待コード</button><button onclick="switchAdminTab(\'stats\',this)" class="sub-nav-btn">統計</button>';
...
function switchAdminTab(tab,btn){document.querySelectorAll('#admin-content .sub-nav-btn').forEach(b=>b.classList.remove('active'));btn.classList.add('active');if(tab==='users')loadAdminUsers();else if(tab==='roles')loadAdminRoles();else if(tab==='tokens')loadAdminTokens();else if(tab==='stats')loadAdminStats();else if(tab==='diag')loadAdminDiagnostics();}
