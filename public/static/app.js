...
function showPushPrompt(){
  showModal('繝励ャ繧ｷ繝･騾夂衍','<div class="text-center space-y-4"><div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-blue-100"><i class="fas fa-bell text-blue-600 text-3xl"></i></div><p class="text-gray-700">縺顔衍繧峨○繧・Γ繝・そ繝ｼ繧ｸ繧偵Μ繧｢繝ｫ繧ｿ繧､繝縺ｧ蜿励￠蜿悶ｌ縺ｾ縺吶・br><strong class="text-blue-600">騾夂衍繧偵が繝ｳ縺ｫ縺吶ｋ縺薙→繧偵♀縺吶☆繧√＠縺ｾ縺吶・/strong></p></div>',[
    {label:'繧ｹ繧ｭ繝・・',className:'border border-gray-300 text-gray-600 px-4 py-2 rounded-xl',action:closeModal},
...
function renderCommittee(container) {
  ...
  const isPE=myCommittee==='菴楢ご蟋泌藤莨・;
  let tabs='';
  if(isStaff) tabs=COMMITTEES.map((c,i)=>'<button class="h-scroll-tab'+(i===0?' active':'')+'" onclick="switchGroupTab(\'committee\',\''+c+'\',this)">'+c+'</button>').join('');
    ...
    if(isPE) tabs+='<button class="h-scroll-tab" onclick="switchGroupTab(\'pe_checklist\',\'\',this)"><i class="fas fa-clipboard-list mr-1"></i>逕ｨ蜈ｷ遒ｺ隱・/button>';
  }
...
async function submitNewSurvey(){if(!surveyQuestions.length){toast('雉ｪ蝠上ｒ1縺､莉･荳願ｿｽ蜉縺励※縺上□縺輔＞','error');return;}try{await api('/api/surveys',{method:'POST',body:{title:surveyQuestions._title,description:surveyQuestions._desc||null,questions:surveyQuestions.map(q=>({text:q.text,type:q.type,options:q.options})),expires_at:surveyQuestions._expires||null}});closeModal();toast('繧｢繝ｳ繧ｱ繝ｼ繝医ｒ菴懈・縺励∪縺励◆・・,'success');loadSurveys('open');}catch(e){toast(e.message||'菴懈・螟ｱ謨・,'error');}}
...
function renderSurveyQuestionAnswer(q,i,prev){if(q.type==='text'){return'<div class="card p-3"><p class="text-sm font-semibold mb-1">'+(i+1)+'. '+esc(q.text)+'</p><textarea class="form-input mt-1" rows="3" data-qid="'+q.id+'" placeholder="閾ｪ逕ｱ險倩ｿｰ">'+esc(prev||'')+'</textarea></div>';}const selected=prev?[].concat(prev):[];const inputType=q.type==='multiple'?'checkbox':'radio';return'<div class="card p-3"><p class="text-sm font-semibold mb-2">'+(i+1)+'. '+esc(q.text)+'</p><div class="space-y-1.5">'+(q.options||[]).map((o,j)=>{const checked=selected.includes(o);return'<label class="flex items-center gap-3 p-2.5 border rounded-xl cursor-pointer hover:bg-purple-50 transition '+(checked?'border-purple-400 bg-purple-50':'')+'"><input type="'+inputType+'" name="sq-'+q.id+'" value="'+esc(o)+'" class="accent-purple-600" '+(checked?'checked':'')+'><span class="text-sm">'+esc(o)+'</span></label>';}).join('')+'</div></div>';}
async function submitSurveyAnswer(sid,qs){const answers=[];let valid=true;document.querySelectorAll('#survey-answer-form .card').forEach((card,i)=>{const q=qs[i];if(!q)return;if(q.type==='text'){const val=card.querySelector('textarea').value.trim();if(!val&&q.type==='text')valid=false;answers.push({question_id:q.id,answer:val||''});}else if(q.type==='multiple'){const vals=Array.from(card.querySelectorAll('input:checked')).map(inp=>inp.value);answers.push({question_id:q.id,answer:vals});}else{const sel=card.querySelector('input:checked');if(!sel)valid=false;else answers.push({question_id:q.id,answer:[sel.value]});}});if(!valid){toast('蜈ｨ縺ｦ縺ｮ雉ｪ蝠上↓蝗樒ｭ斐＠縺ｦ縺上□縺輔＞','error');return;}try{await api('/api/surveys/'+sid+'/answers',{method:'POST',body:{answers}});closeModal();toast('蝗樒ｭ斐ｒ騾∽ｿ｡縺励∪縺励◆・・,'success');loadSurveys('open');}catch(e){toast(e.message||'騾∽ｿ｡螟ｱ謨・,'error');}}

// --- 繧｢繝ｳ繧ｱ繝ｼ繝育ｵ先棡 ---
async function viewSurveyResult(sid){try{const r=await api('/api/surveys/'+sid+'/results');const s=r.survey;let html='<div class="text-sm text-gray-500 mb-3">邱丞屓遲碑・焚: <strong>'+r.totalRespondents+'</strong>莠ｺ</div>';(r.results||[]).forEach((res,i)=>{const q=res.question;html+='<div class="card p-3 mb-3"><p class="text-sm font-bold mb-2">'+(i+1)+'. '+esc(q.text)+'</p>';if(q.type==='text'){html+=(res.answers||[]).map(a=>'<div class="bg-gray-50 rounded-lg p-2 mb-1 text-sm"><span class="text-gray-600">'+esc(a.answer||'')+'</span></div>').join('')||'<p class="text-xs text-gray-400">蝗樒ｭ斐↑縺・/p>';}else{const total=res.total||1;html+=(res.answers||[]).map(a=>{const cnt=a.cnt;const p=((cnt/total)*100).toFixed(1);return'<div class="mb-2"><div class="flex justify-between text-sm mb-0.5"><span>'+esc(a.answer||'')+'</span><span class="text-gray-500">'+cnt+'莠ｺ ('+p+'%)</span></div><div class="w-full h-4 bg-gray-100 rounded-full overflow-hidden"><div class="h-full bg-purple-500 rounded-full transition-all" style="width:'+p+'%"></div></div></div>';}).join('')||'<p class="text-xs text-gray-400">蝗樒ｭ斐↑縺・/p>';}html+='</div>';});showModal(esc(s.title)+' - 邨先棡','<div class="max-h-96 overflow-y-auto">'+html+'</div>',[{label:'髢峨§繧・,className:'border border-gray-300 text-gray-600 px-4 py-2 rounded-xl',action:closeModal}]);}catch(e){toast('隱ｭ霎ｼ螟ｱ謨・,'error');}}
...
async function submitProfileEdit(){try{const name=document.getElementById('pe-name').value.trim();if(!name){toast('蜷榊燕縺ｯ蠢・・,'error');return;}const body={name,club:document.getElementById('pe-club').value.trim()||null,committee:document.getElementById('pe-committee').value.trim()||null,bio:document.getElementById('pe-bio').value.trim()||null,avatar_url:currentUser.avatar_url||null};const r=await api('/api/auth/profile',{method:'PUT',body});closeModal();if(r.pending){toast('螟画峩繝ｪ繧ｯ繧ｨ繧ｹ繝医ｒ騾∽ｿ｡縺励∪縺励◆・域価隱榊ｾ・■・・,'info');}else{toast('菫晏ｭ倥＠縺ｾ縺励◆','success');}loadSettings();}catch(e){toast(e.message||'螟ｱ謨・,'error');}}

// === PE Checklist ===
function renderPEChecklist(container) {
  container.innerHTML='<div><div class="flex gap-2 mb-4"><button onclick="switchPEChecklistTab(\'check\',this)" class="sub-nav-btn active">轤ｹ讀・/button><button onclick="switchPEChecklistTab(\'history\',this)" class="sub-nav-btn">螻･豁ｴ</button><button onclick="switchPEChecklistTab(\'rentals\',this)" class="sub-nav-btn">雋ｸ蜃ｺ</button></div><div id="pe-checklist-content"><div class="skeleton h-32"></div></div></div>';
  loadPEChecklist();
}
...
async function loadPEChecklist(){const c=document.getElementById('pe-checklist-content');if(!c)return;try{const r=await api('/api/checklist/items');c.innerHTML='<div class="space-y-3">'+r.items.map(i=>'<div class="card p-4"><div class="flex justify-between items-start mb-2"><span class="font-bold text-sm">'+esc(i.name)+'</span><span class="text-xs '+(i.status==='ok'?'text-green-600':i.status==='ng'?'text-red-600':'text-gray-400')+'">'+(i.status==='ok'?'笨・OK':i.status==='ng'?'笨・NG':'譛ｪ繝√ぉ繝・け')+'</span></div><p class="text-xs text-gray-500 mb-3">'+esc(i.location||'')+'</p>'+(i.can_check?'<div class="flex gap-2"><button onclick="checkPEItem('+i.id+',\'ok\')" class="bg-green-500 text-white px-4 py-1.5 rounded-full text-xs">OK</button><button onclick="checkPEItem('+i.id+',\'ng\')" class="bg-red-500 text-white px-4 py-1.5 rounded-full text-xs">NG</button></div>':'<p class="text-xs text-gray-400">譛邨ら｢ｺ隱・ '+(i.last_checker?esc(i.last_checker):'')+(i.last_checked?' / '+formatRelative(i.last_checked):'')+'</p>')+'</div>').join('')+'</div>';}catch(e){c.innerHTML='<div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>読込失敗</p><p class="text-xs text-gray-400 mt-1">'+esc(e&&(e.message||e)+'')+'</p></div>';console.error(e);}
...
async function loadPEChecklistHistory(){const c=document.getElementById('pe-checklist-content');if(!c)return;try{const r=await api('/api/checklist/history');if(!r.history.length){c.innerHTML='<div class="empty-state"><i class="fas fa-history"></i><p>螻･豁ｴ縺ｪ縺・/p></div>';return;}c.innerHTML='<div class="space-y-2">'+r.history.map(h=>'<div class="card p-3 flex justify-between items-center"><div><span class="text-sm font-semibold">'+esc(h.item_name)+'</span><span class="text-xs text-gray-500 ml-2">'+esc(h.checker_name)+'</span></div><div class="flex items-center gap-2"><span class="text-xs '+(h.status==='ok'?'text-green-600':'text-red-600')+'">'+h.status.toUpperCase()+'</span><span class="text-xs text-gray-400">'+formatRelative(h.created_at)+'</span></div></div>').join('')+'</div>';}catch{}}




async function loadPERentals(){const c=document.getElementById('pe-checklist-content');if(!c)return;try{const r=await api('/api/checklist/rentals');let h='<div class="mb-3"><button onclick="openRentalModal()" class="bg-blue-600 text-white px-4 py-1.5 rounded-full text-sm font-semibold"><i class="fas fa-plus mr-1"></i>雋ｸ蜃ｺ逋ｻ骭ｲ</button></div>';if(!r.rentals.length){h+='<div class="empty-state"><i class="fas fa-box"></i><p>雋ｸ蜃ｺ螻･豁ｴ縺ｪ縺・/p></div>';c.innerHTML=h;return;}h+='<div class="space-y-2">'+r.rentals.map(rn=>'<div class="card p-3"><div class="flex justify-between"><span class="font-semibold text-sm">'+esc(rn.item_name)+'</span><span class="text-xs '+(rn.returned_at?'text-green-600':'text-yellow-600')+'">'+(rn.returned_at?'霑泌唆貂・:'雋ｸ蜃ｺ荳ｭ')+'</span></div><p class="text-xs text-gray-500">蛟・ '+esc(rn.borrower_name)+' | '+formatRelative(rn.borrowed_at)+'</p>'+(rn.notes?'<p class="text-xs text-gray-400 mt-1">'+esc(rn.notes)+'</p>':'')+(!rn.returned_at?'<button onclick="returnRental('+rn.id+')" class="text-blue-600 text-xs mt-2 hover:underline">霑泌唆</button>':'<p class="text-xs text-gray-400 mt-1">霑泌唆: '+formatRelative(rn.returned_at)+'</p>')+'</div>').join('')+'</div>';c.innerHTML=h;}catch{}}
...
async function renderAdminTeacher(container){
  try{const sr=await api('/api/admin/settings');const s=sr.settings||{};const showTokens=document.createElement('button');showTokens.className='sub-nav-btn active';showTokens.textContent='諡帛ｾ・さ繝ｼ繝・;showTokens.onclick=()=>switchAdminTab('tokens',showTokens);const showStats=document.createElement('button');showStats.className='sub-nav-btn';showStats.textContent='邨ｱ險・;showStats.onclick=()=>switchAdminTab('stats',showStats);const btns=[showTokens,showStats];if(s.teacher_can_users==='true'){const b=document.createElement('button');b.className='sub-nav-btn';b.textContent='繝ｦ繝ｼ繧ｶ繝ｼ邂｡逅・;b.onclick=()=>switchAdminTab('users',b);btns.push(b);}if(s.teacher_can_posts==='true'){const b=document.createElement('button');b.className='sub-nav-btn';b.textContent='謚慕ｨｿ邂｡逅・;b.onclick=()=>switchAdminTab('posts',b);btns.push(b);}if(s.teacher_can_bulk==='true'){const b=document.createElement('button');b.className='sub-nav-btn';b.textContent='荳諡ｬ逕滓・';b.onclick=()=>switchAdminTab('bulk',b);btns.push(b);}container.innerHTML='<div class="bg-white border-b px-4 py-3"><h2 class="font-bold text-gray-800 flex items-center gap-2"><i class="fas fa-shield-alt text-red-600"></i>邂｡逅・/h2></div><div class="p-4 space-y-4"><div class="flex flex-wrap gap-2">'+btns.map(b=>b.outerHTML).join('')+'</div><div id="admin-content"><div class="skeleton h-32"></div></div></div>';loadAdminTokens();}catch{renderAdminFull(container);}
}
...
function renderAdminFull(container){
  ...
  const isStaff=roles.some(r=>['admin','teacher'].includes(r));
  if(!isStaff){container.innerHTML='<div class="empty-state"><i class="fas fa-lock"></i><p>邂｡逅・・・縺ｿ繧｢繧ｯ繧ｻ繧ｹ蜿ｯ閭ｽ</p></div>';return;}
  const tabs=isAdmin?'<button onclick="switchAdminTab(\'users\',this)" class="sub-nav-btn active">繝ｦ繝ｼ繧ｶ繝ｼ邂｡逅・/button><button onclick="switchAdminTab(\'roles\',this)" class="sub-nav-btn">讓ｩ髯千ｮ｡逅・/button><button onclick="switchAdminTab(\'profile\',this)" class="sub-nav-btn">謇ｿ隱榊ｾ・■</button><button onclick="switchAdminTab(\'tokens\',this)" class="sub-nav-btn">諡帛ｾ・さ繝ｼ繝・/button><button onclick="switchAdminTab(\'bulk\',this)" class="sub-nav-btn">荳諡ｬ逕滓・</button><button onclick="switchAdminTab(\'posts\',this)" class="sub-nav-btn">謚慕ｨｿ邂｡逅・/button><button onclick="switchAdminTab(\'stats\',this)" class="sub-nav-btn">邨ｱ險・/button><button onclick="switchAdminTab(\'diag\',this)" class="sub-nav-btn">險ｺ譁ｭ</button>':'<button onclick="switchAdminTab(\'roles\',this)" class="sub-nav-btn active">讓ｩ髯千ｮ｡逅・/button><button onclick="switchAdminTab(\'tokens\',this)" class="sub-nav-btn">諡帛ｾ・さ繝ｼ繝・/button><button onclick="switchAdminTab(\'stats\',this)" class="sub-nav-btn">邨ｱ險・/button>';
...
async function loadAdminRoles(){const c=document.getElementById('admin-content');if(!c)return;try{const r=await api('/api/admin/users');const isAdmin=(currentUser.roles||[currentUser.role]).includes('admin');c.innerHTML='<div class="space-y-2">'+r.users.map(u=>{const roles=(u.all_roles||'').split(',').filter(Boolean);return'<div class="card p-3"><div class="flex justify-between items-center mb-2"><div><span class="font-semibold text-sm">'+esc(u.name)+'</span><span class="text-xs text-gray-500 ml-2">'+esc(u.login_id||u.username||'')+'</span></div><span class="text-xs text-gray-400">'+u.role+(u.grade?' '+u.grade+'-'+u.class_num:'')+'</span></div><div class="flex flex-wrap gap-1 mb-2">'+(roles.length?roles.map(r=>'<span class="inline-flex items-center gap-1 bg-blue-100 text-blue-800 text-xs px-2 py-0.5 rounded-full">'+(ROLE_LABELS[r]||r)+' <button onclick="removeUserRole('+u.id+',\''+r+'\')" class="text-blue-600 hover:text-red-600 ml-0.5">&times;</button></span>').join(''):'<span class="text-xs text-gray-400">繝ｭ繝ｼ繝ｫ縺ｪ縺・/span>')+'</div><div class="flex gap-1 flex-wrap">'+ALL_ROLES.filter(r=>!roles.includes(r)&&r!=='admin').map(r=>'<button onclick="addUserRole('+u.id+',\''+r+'\')" class="border border-gray-300 text-gray-600 hover:bg-blue-50 hover:border-blue-300 px-2 py-0.5 rounded-full text-xs transition">+'+(ROLE_LABELS[r]||r)+'</button>').join('')+'</div></div>';}).join('')+'</div>';}catch(e){c.innerHTML='<div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>読込失敗</p><p class="text-xs text-gray-400 mt-1">'+esc(e&&(e.message||e)+'')+'</p></div>';console.error(e);}
...
async function loadNotifications(){const c=document.getElementById('notif-list');if(!c)return;try{const r=await api('/api/notifications');if(!r.notifications.length){c.innerHTML='<div class="empty-state"><i class="fas fa-bell-slash"></i><p>騾夂衍縺ｯ縺ゅｊ縺ｾ縺帙ｓ</p></div>';return;}c.innerHTML=r.notifications.map(n=>'<div class="card p-4 mb-2 flex items-start gap-3"><div class="w-8 h-8 rounded-full '+(n.is_read?'bg-gray-200':'bg-blue-100')+' flex items-center justify-center text-sm"><i class="fas '+(n.icon||'fa-bell')+' text-blue-600"></i></div><div class="flex-1"><p class="text-sm '+(n.is_read?'text-gray-500':'text-gray-800 font-semibold')+'">'+esc(n.message)+'</p><span class="text-xs text-gray-400">'+formatRelative(n.created_at)+'</span></div>'+(n.is_read?'':'<button onclick="markNotifRead('+n.id+')" class="text-blue-600 text-xs hover:underline">譌｢隱ｭ</button>')+'</div>').join('');}catch{}}
