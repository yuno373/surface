...
function renderSurveyStep2(){const qHtml=surveyQuestions.map((q,i)=>'<div class="flex items-center justify-between bg-gray-50 rounded-lg p-3"><div class="flex-1 min-w-0"><p class="text-sm font-semibold truncate">'+(i+1)+'. '+esc(q.text)+'</p><p class="text-xs text-gray-500">'+{single:'選択式（1つ）',multiple:'選択式（複数）',text:'自由記述'}[q.type]+(q.options?' | 選択肢'+q.options.length+'個':'')+'</p></div><div class="flex gap-1 shrink-0"><button onclick="editSurveyQuestion('+i+')" class="text-blue-600 text-xs hover:underline p-1">編集</button><button onclick="surveyQuestions.splice('+i+',1);renderSurveyStep2()" class="text-red-500 text-xs hover:underline p-1">削除</button></div></div>').join('');showModal('アンケート作成 ②質問作成','<div class="space-y-4"><div class="bg-purple-50 rounded-lg p-3 text-sm text-purple-800"><i class="fas fa-info-circle mr-1"></i>質問を追加してください。選択式（1つ/複数）や自由記述が選べます。</div><div id="survey-questions-list" class="space-y-2">'+qHtml+'</div><button onclick="addSurveyQuestion()" class="w-full border-2 border-dashed border-purple-300 text-purple-600 py-3 rounded-xl text-sm font-semibold hover:bg-purple-50 transition"><i class="fas fa-plus mr-1"></i>質問を追加</button><div class="flex gap-2 justify-between"><button onclick="renderSurveyStep1()" class="border border-gray-300 text-gray-600 px-4 py-2 rounded-xl text-sm"><i class="fas fa-arrow-left mr-1"></i>戻る</button><button onclick="submitNewSurvey()" class="bg-green-600 text-white px-6 py-2 rounded-xl text-sm font-semibold"><i class="fas fa-check mr-1"></i>アンケートを作成</button></div></div>',[]);}
...
async function submitNewSurvey(){if(!surveyQuestions.length){toast('質問を1つ以上追加してください','error');return;}try{await api('/api/surveys',{method:'POST',body:{title:surveyQuestions._title,description:surveyQuestions._desc||null,questions:surveyQuestions.map(q=>({text:q.text,type:q.type,options:q.options})),expires_at:surveyQuestions._expires||null}});closeModal();toast('アンケートを作成しました！','success');loadSurveys('open');}catch(e){toast(e.message||'作成失敗','error');}}
...
// --- アンケート結果 ---
async function viewSurveyResult(sid){try{const r=await api('/api/surveys/'+sid+'/results');const s=r.survey;let html='<div class="text-sm text-gray-500 mb-3">総回答者数: <strong>'+r.totalRespondents+'</strong>人</div>';(r.results||[]).forEach((res,i)=>{const q=res.question;html+='<div class="card p-3 mb-3"><p class="text-sm font-bold mb-2">'+(i+1)+'. '+esc(q.text)+'</p>';if(q.type==='text'){html+=(res.answers||[]).map(a=>'<div class="bg-gray-50 rounded-lg p-2 mb-1 text-sm"><span class="text-gray-600">'+esc(a.answer||'')+'</span></div>').join('')||'<p class="text-xs text-gray-400">回答なし</p>';}else{const total=res.total||1;html+=(res.answers||[]).map(a=>{const cnt=a.cnt;const p=((cnt/total)*100).toFixed(1);return'<div class="mb-2"><div class="flex justify-between text-sm mb-0.5"><span>'+esc(a.answer||'')+'</span><span class="text-gray-500">'+cnt+'人 ('+p+'%)</span></div><div class="w-full h-4 bg-gray-100 rounded-full overflow-hidden"><div class="h-full bg-purple-500 rounded-full transition-all" style="width:'+p+'%"></div></div></div>';}).join('')||'<p class="text-xs text-gray-400">回答なし</p>';}html+='</div>';});showModal(esc(s.title)+' - 結果','<div class="max-h-96 overflow-y-auto">'+html+'</div>',[{label:'閉じる',className:'border border-gray-300 text-gray-600 px-4 py-2 rounded-xl',action:closeModal}]);}catch(e){toast('読込失敗','error');}}
...
async function renderAdminTeacher(container){
  try{const sr=await api('/api/admin/settings');const s=sr.settings||{};const showRoles=document.createElement('button');showRoles.className='sub-nav-btn active';showRoles.textContent='権限管理';showRoles.onclick=()=>switchAdminTab('roles',showRoles);const showTokens=document.createElement('button');showTokens.className='sub-nav-btn';showTokens.textContent='招待コード';showTokens.onclick=()=>switchAdminTab('tokens',showTokens);const showStats=document.createElement('button');showStats.className='sub-nav-btn';showStats.textContent='統計';showStats.onclick=()=>switchAdminTab('stats',showStats);const btns=[showRoles,showTokens,showStats];if(s.teacher_can_users==='true'){const b=document.createElement('button');b.className='sub-nav-btn';b.textContent='ユーザー管理';b.onclick=()=>switchAdminTab('users',b);btns.push(b);}if(s.teacher_can_posts==='true'){const b=document.createElement('button');b.className='sub-nav-btn';b.textContent='投稿管理';b.onclick=()=>switchAdminTab('posts',b);btns.push(b);}if(s.teacher_can_bulk==='true'){const b=document.createElement('button');b.className='sub-nav-btn';b.textContent='一括生成';b.onclick=()=>switchAdminTab('bulk',b);btns.push(b);}container.innerHTML='<div class="bg-white border-b px-4 py-3"><h2 class="font-bold text-gray-800 flex items-center gap-2"><i class="fas fa-shield-alt text-red-600"></i>管理</h2></div><div class="p-4 space-y-4"><div class="flex flex-wrap gap-2">'+btns.map(b=>b.outerHTML).join('')+'</div><div id="admin-content"><div class="skeleton h-32"></div></div></div>';loadAdminRoles();}catch{renderAdminFull(container);}
}
...
function renderAdminFull(container){
  ...
  const isStaff=roles.some(r=>['admin','teacher'].includes(r));
  if(!isStaff){container.innerHTML='<div class="empty-state"><i class="fas fa-lock"></i><p>管理者のみアクセス可能</p></div>';return;}
  const tabs=isAdmin?'<button onclick="switchAdminTab(\'users\',this)" class="sub-nav-btn active">ユーザー管理</button><button onclick="switchAdminTab(\'roles\',this)" class="sub-nav-btn">権限管理</button><button onclick="switchAdminTab(\'profile\',this)" class="sub-nav-btn">承認待ち</button><button onclick="switchAdminTab(\'tokens\',this)" class="sub-nav-btn">招待コード</button><button onclick="switchAdminTab(\'bulk\',this)" class="sub-nav-btn">一括生成</button><button onclick="switchAdminTab(\'posts\',this)" class="sub-nav-btn">投稿管理</button><button onclick="switchAdminTab(\'stats\',this)" class="sub-nav-btn">統計</button><button onclick="switchAdminTab(\'diag\',this)" class="sub-nav-btn">診断</button>':'<button onclick="switchAdminTab(\'roles\',this)" class="sub-nav-btn active">権限管理</button><button onclick="switchAdminTab(\'tokens\',this)" class="sub-nav-btn">招待コード</button><button onclick="switchAdminTab(\'stats\',this)" class="sub-nav-btn">統計</button>';
