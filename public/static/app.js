...
function toggleSurveyTargetField(){var v=document.getElementById('st1-target');var f=document.getElementById('st1-target-value-field');var l=document.getElementById('st1-target-label');var c=document.getElementById('st1-target-value-container');if(!v||!f)return;var show=v.value!=='all';f.classList.toggle('hidden',!show);if(show){var labels={class:'クラス',club:'部活',committee:'委員会'};l.textContent=labels[v.value]||'';if(!c.querySelector('select')){if(v.value==='class'){var sel=document.createElement('select');sel.id='st1-target-value';sel.className='form-input';sel.innerHTML='<option value="">読み込み中...</option>';c.innerHTML='';c.appendChild(sel);fetch('/api/admin/classes').then(function(r){return r.json();}).then(function(r){var classes=r.classes||[];sel.innerHTML=classes.map(function(x){return'<option value="'+x.grade+'-'+x.class_num+'">'+x.grade+'年'+x.class_num+'組</option>';}).join('')||'<option value="">クラスが見つかりません</option>';}).catch(function(){sel.innerHTML='<option value="">読込失敗</option>';});}else{var opts=v.value==='club'?CLUBS:COMMITTEES;c.innerHTML='<select id="st1-target-value" class="form-input">'+opts.map(function(o){return'<option value="'+esc(o)+'">'+esc(o)+'</option>';}).join('')+'</select>';}}}}
function openNewSurveyModal(){surveyQuestions=[];renderSurveyStep1();}
function renderSurveyStep1(){const targetVal=surveyQuestions._target||'all';const targetValV=surveyQuestions._targetValue||'';const showTarget=targetVal!=='all';showModal('アンケート作成 ①基本情報','<div class="space-y-4"><div class="bg-purple-50 rounded-lg p-3 text-sm text-purple-800"><i class="fas fa-info-circle mr-1"></i>まずはアンケートの基本情報を入力してください</div><div><label class="form-label">タイトル <span class="text-red-500">*</span></label><input id="st1-title" type="text" class="form-input" placeholder="例: 体育祭の種目について" value="'+(surveyQuestions._title||'')+'"></div><div><label class="form-label">説明（任意）</label><textarea id="st1-desc" class="form-input" rows="2" placeholder="アンケートの目的などを記載">'+(surveyQuestions._desc||'')+'</textarea></div><div><label class="form-label">回答期限（任意）</label><input id="st1-expires" type="date" class="form-input" value="'+(surveyQuestions._expires||'')+'"></div><div><label class="form-label">配信対象</label><select id="st1-target" class="form-input" onchange="toggleSurveyTargetField()"><option value="all"'+(targetVal==='all'?' selected':'')+'>全校</option><option value="class"'+(targetVal==='class'?' selected':'')+'>クラス</option><option value="club"'+(targetVal==='club'?' selected':'')+'>部活</option><option value="committee"'+(targetVal==='committee'?' selected':'')+'>委員会</option></select></div><div id="st1-target-value-field" class="'+(showTarget?'':'hidden')+'"><label class="form-label" id="st1-target-label">'+(targetVal==='class'?'クラス':(targetVal==='club'?'部活':(targetVal==='committee'?'委員会':'')))+'</label><div id="st1-target-value-container"><input id="st1-target-value" type="text" class="form-input" placeholder="選択してください..." value="'+esc(targetValV)+'"></div></div><div class="flex gap-2 justify-end"><button onclick="closeModal()" class="border border-gray-300 text-gray-600 px-4 py-2 rounded-xl text-sm">\u30ad\u30e3\u30f3\u30bb\u30eb</button><button onclick="saveSurveyStep1()" class="bg-purple-600 text-white px-6 py-2 rounded-xl text-sm font-semibold">\u6b21\u3078 \u2461\u8cea\u554f\u4f5c\u6210 <i class="fas fa-arrow-right ml-1"></i></button></div></div>',[]);setTimeout(toggleSurveyTargetField,0);}
...
async function loadAdminDiagnostics(){const c=document.getElementById('admin-content');if(!c)return;c.innerHTML='<div class="space-y-4"><div class="flex gap-2"><button onclick="loadAdminDiagnostics()" class="bg-blue-600 text-white px-4 py-1.5 rounded-full text-sm"><i class="fas fa-sync mr-1"></i>再診断</button></div><div id="diag-results"><div class="skeleton h-32"></div></div></div>';try{const r=await api('/api/admin/diagnostics');const l=document.getElementById('diag-results');var ok=0,err=0,info=0;r.checks.forEach(function(ch){if(ch.status==='ok')ok++;else if(ch.status==='error')err++;else info++;});var summary='';if(err>0)summary='<div class="flex items-center gap-3 p-3 bg-red-50 rounded-lg mb-3"><i class="fas fa-exclamation-circle text-red-500 text-lg"></i><span class="text-sm font-semibold text-red-700">'+err+'件のエラー</span><span class="text-sm text-gray-500">| OK: '+ok+' | 情報: '+info+'</span></div>';else summary='<div class="flex items-center gap-3 p-3 bg-green-50 rounded-lg mb-3"><i class="fas fa-check-circle text-green-500 text-lg"></i><span class="text-sm font-semibold text-green-700">全'+r.checks.length+'件 OK</span></div>';l.innerHTML=summary+'<div class="space-y-2">'+r.checks.map(function(ch){var icon={ok:'fa-check-circle text-green-500',error:'fa-exclamation-circle text-red-500',info:'fa-info-circle text-blue-500'}[ch.status]||'fa-circle text-gray-400';return'<div class="card p-3 flex items-center gap-3"><i class="fas '+icon+'"></i><div class="flex-1"><p class="text-sm font-semibold">'+esc(ch.name)+'</p><p class="text-xs text-gray-500">'+esc(ch.message)+'</p></div></div>';}).join('')+'</div>';}catch{const l=document.getElementById('diag-results');if(l)l.innerHTML='<div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>診断失敗</p></div>';}}
