...
async function showWeatherModal(){const body=document.getElementById('modal-body');const title=document.getElementById('modal-title');if(!body||!title)return;title.textContent='天気予報';showModal('天気予報','<div class="skeleton h-48"></div>',[]);try{const r=await api('/api/weather/forecast');const f=r.forecasts?.[0];if(!f){document.getElementById('modal-body').innerHTML='<p class="text-sm text-gray-400">予報データが取得できませんでした</p>';return;}const popsHtml=f.pops&&f.pops.length?'<div><p class="text-sm font-semibold mb-1">降水確率</p><div class="flex gap-2">'+['06時','12時','18時','24時'].map(function(t,i){return'<div class="flex-1 text-center"><div class="text-xs text-gray-500">'+t+'</div><div class="text-lg font-bold">'+(f.pops[i]||'--')+'%</div></div>';}).join('')+'</div></div>':'';const tempsHtml=f.temps?'<div><p class="text-sm font-semibold mb-1">気温</p><div class="flex gap-2">'+(f.temps.today?'<div class="flex-1 text-center"><div class="text-xs text-gray-500">日中</div><div class="text-lg font-bold">'+f.temps.today+'°C</div></div>':'')+(f.temps.tonight?'<div class="flex-1 text-center"><div class="text-xs text-gray-500">夜</div><div class="text-lg font-bold">'+f.temps.tonight+'°C</div></div>':'')+'</div></div>':'';document.getElementById('modal-body').innerHTML='<div class="space-y-4"><div class="card p-4"><div class="text-lg">'+esc(f.weather||'')+'</div><p class="text-xs text-gray-400 mt-1">'+esc(f.wind||'')+'</p></div>'+popsHtml+tempsHtml+'<div class="card p-4"><p class="text-sm font-semibold mb-2">明日の予報</p><div class="text-lg">'+esc(f.weatherTomorrow||'')+'</div><p class="text-xs text-gray-400 mt-1">'+esc(f.windTomorrow||'')+'</p></div><p class="text-xs text-gray-400">'+esc(f.publishingOffice||'')+' | '+esc((f.reportDatetime||'').slice(0,16).replace('T',' '))+'</p></div>';}catch{document.getElementById('modal-body').innerHTML='<p class="text-sm text-gray-400">読込失敗</p>';}}async function fetchUnreadCount(){try{const r=await api('/api/messages/unread-count');const badge=document.getElementById('msg-badge');if(badge){badge.textContent=r.count>0?(r.count>99?'99+':r.count):'';badge.classList.toggle('hidden',r.count===0);}}catch{}}
