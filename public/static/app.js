...
async function fetchWBGT(){const el=document.getElementById('wbgt-text');const tm=setTimeout(function(){if(el&&el.textContent==='気象情報を取得中...')el.textContent='気象情報取得失敗';},8000);try{const r=await api('/api/wbgt');clearTimeout(tm);if(el){if(r.wbgt){const levelMap={'危険':'text-red-300','厳重警戒':'text-yellow-300','警戒':'text-yellow-200','注意':'text-green-200'};el.innerHTML=(r.weather?r.weather+' ':'')+'<strong>'+r.temp+'°C</strong> 湿度'+r.humidity+'% | WBGT: <strong>'+r.wbgt+'°C</strong> <span class="'+(levelMap[r.level]||'')+'">('+r.level+')</span>'+(r.alert?' <span class="text-yellow-200">⚠'+r.alert+'</span>':'');}else{el.textContent='気象情報取得失敗';}}}catch{clearTimeout(tm);if(el)el.textContent='気象情報取得失敗';}}
...
async function showWeatherModal(){showModal('天気予報','<div class="skeleton h-48"></div>',[]);try{const r=await api('/api/weather/forecast');const f=r.forecasts?.[0];if(!f){document.getElementById('modal-body').innerHTML='<p class="text-sm text-gray-400 p-4">読込失敗</p>';return;}const pops=f.pops||[];const wd=f.weather||'';const wd2=f.weatherTomorrow||'';const h='<div class="space-y-4 p-2">'+(wd?'<div class="card p-4"><div class="text-lg font-bold mb-1">今日の天気</div><p class="text-sm leading-relaxed">'+(function(t){if(!t)return'';var parts=t.split(/\u3000/g).filter(Boolean);if(!parts.length)return'';var lines=[],cur='';var timeRe=/^(朝|昼前|昼過ぎ|昼頃|昼|夕方|夜|明け方|未明|主に)/;for(var i=0;i<parts.length;i++){var s=parts[i];if(timeRe.test(s)&&cur){lines.push(cur);cur=s;}else if(cur){cur=cur+' '+s;}else{cur=s;}}if(cur)lines.push(cur);return lines.map(function(s,i){var cls=i===0?'font-semibold text-base':'text-gray-600 text-sm mt-0.5';return'<div class="'+cls+'">'+s+'</div>';}).join('');})(wd)+'</p>'+(f.wind?'<p class="text-xs text-gray-500 mt-1">風: '+esc(f.wind)+'</p>':'')+'</div>':'')+(pops.length?'<div class="card p-4"><div class="text-sm font-bold mb-2">降水確率</div><div class="grid grid-cols-4 gap-1 text-center">'+['6-12時','12-18時','18-24時','24-6時'].map((t,i)=>'<div><div class="text-xs text-gray-500">'+t+'</div><div class="text-xl font-bold'+(parseInt(pops[i])>=50?' text-blue-600':'')+'">'+(pops[i]||'-')+'%</div></div>').join('')+'</div></div>':'')+(f.temps?'<div class="card p-4"><div class="text-sm font-bold mb-2">気温</div><div class="grid grid-cols-2 gap-2 text-center">'+(f.temps.today?'<div><div class="text-xs text-gray-500">日中</div><div class="text-xl font-bold">'+f.temps.today+'°C</div></div>':'')+(f.temps.tonight?'<div><div class="text-xs text-gray-500">夜</div><div class="text-xl font-bold">'+f.temps.tonight+'°C</div></div>':'')+'</div></div>':'')+(wd2?'<div class="card p-4"><div class="text-sm font-bold mb-1">明日の天気</div><p class="text-sm leading-relaxed">'+(function(t){if(!t)return'';var parts=t.split(/\u3000/g).filter(Boolean);if(!parts.length)return'';var lines=[],cur='';var timeRe=/^(朝|昼前|昼過ぎ|昼頃|昼|夕方|夜|明け方|未明|主に)/;for(var i=0;i<parts.length;i++){var s=parts[i];if(timeRe.test(s)&&cur){lines.push(cur);cur=s;}else if(cur){cur=cur+' '+s;}else{cur=s;}}if(cur)lines.push(cur);return lines.map(function(s,i){var cls=i===0?'font-semibold text-base':'text-gray-600 text-sm mt-0.5';return'<div class="'+cls+'">'+s+'</div>';}).join('');})(wd2)+'</p>'+(f.windTomorrow?'<p class="text-xs text-gray-500 mt-1">風: '+esc(f.windTomorrow)+'</p>':'')+'</div>':'')+(f.publishingOffice?'<p class="text-xs text-gray-400 text-center">'+esc(f.publishingOffice)+' | '+(f.reportDatetime||'').slice(0,16).replace('T',' ')+'</p>':'')+'</div>';document.getElementById('modal-body').innerHTML=h;}catch{document.getElementById('modal-body').innerHTML='<p class="text-sm text-gray-400 p-4">読込失敗</p>';}}async function fetchUnreadCount(){try{const r=await api('/api/messages/unread-count');const badge=document.getElementById('msg-badge');if(badge){badge.textContent=r.count>0?(r.count>99?'99+':r.count):'';badge.classList.toggle('hidden',r.count===0);}}catch{}}
