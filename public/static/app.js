...
async function loadTVPosts(){var c=document.getElementById('tv-content');if(!c)return;try{var r=await Promise.all([api('/api/posts?category=committee'),api('/api/posts?category=club')]);var cp=r[0].posts||[],cbp=r[1].posts||[];var renderCol=function(posts,title,color){var h='<div style="display:flex;flex-direction:column;overflow-y:auto;padding:16px;background:#0f172a;border-right:1px solid #1e293b">';h+='<h2 style="font-size:22px;font-weight:bold;margin-bottom:12px;padding-bottom:8px;border-bottom:3px solid '+color+';color:'+color+';flex-shrink:0"><i class="fas '+(title.indexOf('委員会')>=0?'fa-users-cog':'fa-running')+' mr-2"></i>'+title+'</h2>';if(!posts.length){h+='<p style="color:#64748b;font-size:16px;text-align:center;padding:32px">投稿はありません</p>';}else{h+=posts.map(function(p){return'<div style="background:#1e293b;border-radius:12px;padding:14px;margin-bottom:10px;border-left:4px solid '+color+';font-size:16px;line-height:1.6">'+(p.author_name?'<div style="font-size:13px;color:#94a3b8;margin-bottom:4px"><strong style="color:#cbd5e1">'+esc(p.author_name)+'</strong><span style="margin-left:8px">'+formatRelative(p.created_at)+'</span></div>':'')+(p.title?'<h3 style="font-size:18px;font-weight:bold;margin-bottom:4px;color:#f1f5f9">'+esc(p.title)+'</h3>':'')+'<p style="color:#cbd5e1;font-size:16px">'+esc(p.content)+'</p></div>';}).join('');}h+='</div>';return h;};c.innerHTML=renderCol(cp,'委員会連絡','#a855f7')+renderCol(cbp,'部活動連絡','#ef4444');}catch{c.innerHTML='<div style="grid-column:1/-1;text-align:center;padding:60px;font-size:18px;color:#64748b">読み込みに失敗しました</div>';}}
